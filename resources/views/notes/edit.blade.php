                ...
                <form id="edit-note-form" method="POST" action="{{ route('notes.update', $note) }}" enctype="multipart/form-data" class="p-6" data-upload-progress data-no-loader>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" name="pages" id="pages" value="{{ old('pages', $note->pages) }}"
                    ...
                    {{-- Nuevos adjuntos --}}
                    <div class="mt-5" x-data="fileUpload({ maxMB: 5000, maxFiles: 10, acceptedExtensions: ['.pdf', '.doc', '.docx', '.zip', '.rar'], acceptedLabel: 'PDF, Word, ZIP o RAR' })">
                        ...
                        <div class="mt-1 border-2 border-dashed rounded-xl p-6 text-center transition-all duration-200 cursor-pointer"
                            ...
                            <p class="text-xs mt-1" style="color: var(--text-muted);">PDF, Word (DOC/DOCX), ZIP y RAR &mdash; máximo 10 archivos, 5000MB por archivo</p>
                            <input x-ref="fileInput" type="file" name="attachments[]" multiple accept=".pdf,.doc,.docx,.zip,.rar,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/zip,application/x-zip-compressed,application/x-rar-compressed,application/vnd.rar,application/octet-stream" class="hidden" @change="handleFiles($event)">
                        </div>
