...
<div x-data="notesPage()" @keydown.escape.window="close()">
                            ...
                            @forelse($notes as $index => $note)
                                ...
                                <tr class="group transition-colors duration-150 hover:bg-blue-50/40 dark:hover:bg-blue-900/10">
                                    ...
                                    <td class="px-3 py-2.5">
                                        <button type="button" @click="openView(@js($noteData))" class="text-xs font-bold hover:underline" style="color: var(--abc-navy);">
                                            {{ $note->internal_number }}
                                        </button>
                                    </td>
                                    <td class="px-3 py-2.5 text-xs whitespace-nowrap" style="color: var(--text-secondary);">{{ $note->note_date->format('d/m/Y') }}</td>
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-md ring-1 ring-inset {{ $docTypeClass }}">{{ $note->doc_type }}</span>
                                    </td>
                                    {{-- H: FOJAS --}}
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        @if($note->note_type)
                                            <span class="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-md ring-1 ring-inset bg-indigo-50 text-indigo-700 ring-indigo-200" title="{{ $note->note_type }}">
                                                {{ \Illuminate\Support\Str::limit($note->note_type, 18) }}
                                            </span>
                                        @else
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        @if($note->tipologia)
                                            <span class="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-md ring-1 ring-inset bg-teal-50 text-teal-700 ring-teal-200" title="{{ $note->tipologia }}">
                                                {{ \Illuminate\Support\Str::limit($note->tipologia, 16) }}
                                            </span>
                                        @else
                                            <span class="text-[10px]" style="color: var(--text-muted);">-</span>
                                        @endif
                                    ...
                                    <td class="px-3 py-2.5 text-center">
                                        @if($note->estado_conservacion)
                                            ...
                                            @endphp
                                            <span class="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-md ring-1 ring-inset {{ $conservColors }}" title="{{ $note->estado_conservacion }}">
                                                {{ \Illuminate\Support\Str::limit($note->estado_conservacion, 14) }}
                                            </span>
                                        @else
                                            <span class="text-[10px]" style="color: var(--text-muted);">-</span>
                                        @endif
                                    ...
                                    <td class="px-2 py-2.5 text-center sticky right-0 transition-colors duration-150" style="background-color: var(--surface-card); box-shadow: -4px 0 8px -4px rgba(0,0,0,0.08);">
                                        <div class="inline-flex items-center gap-1.5">
                                            {{-- Ver --}}
                                            <button type="button" @click="openView(@js($noteData))"
                                            ...
                                            @can('update', $note)
                                                {{-- Editar --}}
                                                <button type="button" @click="openEdit(@js($noteData))"
                    ...
                    <div class="mobile-card-item">
                        ...
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-2">
                                ...
                                <button type="button" @click="openView(@js($mobileData))" class="text-sm font-bold hover:underline" style="color: var(--accent-primary);">
                                    {{ $note->internal_number }}
                                </button>
                            </div>
                            @include('partials.status-badge', ['status' => $note->status])
                        ...
                        <div class="grid grid-cols-2 gap-x-4 gap-y-1">
                            ...
                            <div class="mobile-card-row">
                                ...
                                <span class="mobile-card-value text-xs truncate">{{ $note->tipologia }}</span>
                            </div>
                            @endif
                            @if($note->estado_conservacion)
                            <div class="mobile-card-row">
                                <span class="mobile-card-label">Conservación</span>
                                <span class="mobile-card-value text-xs truncate">{{ $note->estado_conservacion }}</span>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="mobile-card-actions grid grid-cols-3 gap-2 mt-3">
                            <button type="button" @click="openView(@js($mobileData))"
                            ...
                            @can('update', $note)
                                <button type="button" @click="openEdit(@js($mobileData))"
                            ...
                            @endcan
                            @can('delete', $note)
                                <form method="POST" action="{{ route('notes.destroy', $note) }}" id="mobile-delete-{{ $note->id }}" onsubmit="event.preventDefault(); confirmarEliminar('{{ $note->internal_number }}', 'mobile-delete-{{ $note->id }}')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
    ...
    {{-- ═══════════════════════════════════════════════════════════
         MODAL DE VER (solo lectura, profesional)
         ═══════════════════════════════════════════════════════════ --}}
        ...
        <div class="min-h-screen flex items-center justify-center p-4">
            <div x-show="viewing" x-cloak
                ...
                <div class="max-h-[70vh] overflow-y-auto">
                    ...
                    {{-- Grid principal con todos los campos Excel --}}
                    <div class="p-6">
    ...
    {{-- ═══════════════════════════════════════════════════════════
         MODAL DE EDITAR (formulario completo)
         ═══════════════════════════════════════════════════════════ --}}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select name="box_id" class="abc-input" required x-model="editing.box_id">
                                    ...
                                    @foreach($boxes as $box)
                                        <option value="{{ $box->id }}">{{ $box->box_number }} - {{ Str::limit($box->description, 40) }}</option>
                                    @endforeach
                                </select>
                            </div>
