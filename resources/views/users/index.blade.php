                    ...
                    <div class="mobile-card-item" x-data="{ showReset: false }">
                        <div class="flex items-center gap-3 mb-3">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-bold text-sm truncate" style="color: var(--text-primary)">{{ $user->name }}</p>
                        ...
                        <div class="grid grid-cols-2 gap-x-4 gap-y-1 mb-3">
                            ...
                            <div class="mobile-card-row">
                                ...
                                <span class="mobile-card-value text-xs font-mono">{{ $user->id }}</span>
                            </div>
                        </div>
                        <div class="mobile-card-actions">
                            <a href="{{ route('users.edit', $user) }}" class="text-amber-600 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-400 hover:bg-amber-100 rounded-lg">
                            ...
                            @if(auth()->user()->isAdmin() && $user->id !== auth()->id())
                                <form method="POST" action="{{ route('users.destroy', $user) }}" class="inline" id="mobile-delete-user-{{ $user->id }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                            onclick="confirmarEliminarUsuario('{{ addslashes($user->name) }}', 'mobile-delete-user-{{ $user->id }}')"
